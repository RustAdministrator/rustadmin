...
void androidChannelInit() {
  gFFI.setMethodCallHandler((method, arguments) {
    if (method != 'remote_keyboard_event') {
      debugPrint("flutter got android msg,$method,$arguments");
    }
    try {
      switch (method) {
        case 'remote_keyboard_event':
          {
            final event = AndroidRemoteKeyboardEvent.tryParse(arguments);
            if (event == null || event.sessionId != gFFI.sessionId.toString()) {
              break;
            }
            switch (event) {
        ...
        case "outgoing_session_closed":
          {
            AndroidVpnSessionCoordinator.instance.handleNativeSessionClosed(
              arguments,
            );
            break;
          }
