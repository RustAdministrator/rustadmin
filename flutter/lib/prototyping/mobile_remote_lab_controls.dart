...
class _MobileRemoteToolbarState extends State<MobileRemoteToolbar> {
  ...
  // The Lab deliberately owns its visual defaults.  Production settings are
  // copied in only when a Lab experiment is explicitly promoted.
...
class MobileRemoteKeyHelpTools extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: _LabKeyHelpScrollStrip(buttons: allButtons, spacing: spacing),
    );
  }
}

class _LabKeyHelpScrollStrip extends StatefulWidget {
  const _LabKeyHelpScrollStrip({required this.buttons, required this.spacing});

  final List<Widget> buttons;
  final double spacing;

  @override
  State<_LabKeyHelpScrollStrip> createState() => _LabKeyHelpScrollStripState();
}

class _LabKeyHelpScrollStripState extends State<_LabKeyHelpScrollStrip> {
  ...
  void _onPointerMove(PointerMoveEvent event) {
    if (event.pointer != _mousePointer || !_controller.hasClients) return;
    final position = _controller.position;
    final target = (position.pixels - event.delta.dx)
        .clamp(0.0, position.maxScrollExtent)
        .toDouble();
    _controller.jumpTo(target);
  }

  void _clearMousePointer(PointerEvent event) {
    if (event.pointer == _mousePointer) _mousePointer = null;
  }
  ...
  Widget build(BuildContext context) {
    return Listener(
      ...
      onPointerUp: _clearMousePointer,
      onPointerCancel: _clearMousePointer,
...
class _MobileRemoteOptionsContentState
  ...
  Widget _backHeader(BuildContext context, String title, VoidCallback onBack) {
    return Padding(
      ...
      child: Row(
        children: [
          IconButton(
            key: const Key('mobile-lab-options-back'),
  ...
  Widget _buildRoot(BuildContext context) {
    ...
    for (final section in widget.radioSections) {
      ...
      children.add(
        _rootMenuItem(
          key: Key('mobile-lab-options-open-${section.id}'),
    ...
    if (widget.toggles.isNotEmpty) {
      children.add(
        _rootMenuItem(
          key: const Key('mobile-lab-options-open-session-controls'),
    ...
    return Column(
      key: const Key('mobile-lab-options-root'),
      mainAxisSize: MainAxisSize.min,
      children: children,
    );
  }
  ...
  Widget _buildRadioSection(
    ...
    return Column(
      key: Key('mobile-lab-options-submenu-${section.id}'),
      mainAxisSize: MainAxisSize.min,
      children: [
        _backHeader(
          context,
  ...
  Widget _buildToggles(BuildContext context) {
    return Column(
      key: const Key('mobile-lab-options-session-controls'),
      mainAxisSize: MainAxisSize.min,
      children: [
        _backHeader(
          context,
