...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Send clipboard to peer only", "Только отправлять удалённому устройству"),
        ("Receive clipboard from peer only", "Только получать от удалённого устройства"),
        ("Disabled", "Отключено"),
        ...
        ("Display", "Отображение"),
        ("Capture", "Захват"),
        ("Quality monitor", "Монитор качества"),
        ("Quality monitor position", "Положение монитора качества"),
        ("Quality monitor details", "Детали монитора качества"),
        ...
        ("Transport", "Транспорт"),
        ("Auto (QUIC preferred)", "Авто (предпочитать QUIC)"),
        ("QUIC only", "Только QUIC"),
        ("TCP only", "Только TCP"),
        ("VPN", "VPN"),
        ("Start WireGuard when unavailable", "Запускать WireGuard, если узел недоступен"),
        ("WireGuard tunnel name", "Имя туннеля WireGuard"),
        ("WireGuard tunnel name is required", "Укажите имя туннеля WireGuard"),
        ("Checking peer availability and VPN", "Проверка доступности узла и VPN"),
        ("Another VPN connection check is already running", "Уже выполняется другая проверка VPN"),
        ("WireGuard tunnel name is not configured", "Имя туннеля WireGuard не настроено"),
        ("WireGuard is not installed", "WireGuard не установлен"),
        ("WireGuard control permission was not granted", "Разрешение на управление WireGuard не предоставлено"),
        ("Failed to request WireGuard tunnel start", "Не удалось запросить запуск туннеля WireGuard"),
        ("WireGuard did not make the peer reachable. Check the tunnel name and Allow remote control apps in WireGuard", "После запуска WireGuard узел остался недоступен. Проверьте имя туннеля и параметр Allow remote control apps в WireGuard"),
        ("WireGuard tunnel did not become active. Check the tunnel name and Allow remote control apps in WireGuard", "Туннель WireGuard не активировался. Проверьте имя туннеля и параметр Allow remote control apps в WireGuard"),
        ("WireGuard is active but did not make the peer reachable. Check the peer address and tunnel routes", "WireGuard активен, но узел остался недоступен. Проверьте адрес узла и маршруты туннеля"),
        ("VPN connection preparation failed", "Не удалось подготовить VPN-подключение"),
        ("Session closed after 10 minutes in background", "Сеанс закрыт после 10 минут в фоне"),
        ("enable-quic-tip", "Выберите транспорт удалённого сеанса. Автоматический режим предпочитает QUIC через любой маршрутизируемый UDP-путь и переходит на TCP только при недоступности UDP или слишком старом узле. При ошибке сертификата или идентификации узла переход запрещён."),
