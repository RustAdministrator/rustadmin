...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("codec_direction_tip", "Encoder is this device's hosting default; Decoder is its viewing default. Auto on both sides prefers AV1. If only one side selects a format, that format is preferred; conflicting explicit selections favor the viewer. Prefer hardware codec tries hardware first; unchecked tries software first. Either can fall back to the other for the same format, when available in this build. Defaults apply to new connections; the toolbar changes the current connection."),
        ("decoder_preference_tip", "Selects decoding on this device. Only the video format is requested from the remote computer; it chooses its own encoder. HW tries a validated hardware decoder first; SW tries software first. Either can fall back to the other for the same format. Runtime fallback is shown in Quality monitor."),
