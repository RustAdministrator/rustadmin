...
usage() {
  cat <<'USAGE'
Usage: scripts/run_toolbar_lab_linux.sh [--clean] [--hwcodec] [--skip-cargo] [--device DEVICE] [--target DART_TARGET] [-- FLUTTER_RUN_ARGS...]
...
lab_mode="${RUSTADMIN_FLUTTER_LAB:-0}"
...
export PKG_CONFIG_PATH="$repo_root/pkgconfig:${PKG_CONFIG_PATH:-}"
...
if [[ -n "$deps_root" && ! -e "$deps_root" ]]; then
